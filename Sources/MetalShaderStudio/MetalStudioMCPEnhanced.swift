    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 0) {
                    // Top section: Editor (left) and Preview (right) - FIXED HEIGHTS
                    HStack(spacing: 0) {
                        // Shader Editor - FIXED WIDTH
                        ShaderEditorView()
                            .frame(width: 400, height: showLibrary ? 350 : 500)
                        ...
                        VStack(spacing: 0) {
                            MetalPreviewView()
                                .padding(.top, 8)
                                .frame(width: 480, height: showConsole && !showLibrary ? 200 : (showLibrary ? 350 : 500))
                        ...
                        // Right Panel - Inspector/MCP - FIXED WIDTH
                        if showInspector || showMCPPanel {
                            ...
                            VStack(spacing: 0) {
                                if showMCPPanel {
                                    MCPControlPanel()
                                        .frame(width: 320, height: showInspector ? 250 : (showLibrary ? 350 : 500))
                                ...
                                if showInspector {
                                    InspectorView()
                                        .frame(width: 320, height: showMCPPanel ? 250 : (showLibrary ? 350 : 500))
                                }
                            }
                        }
                    }
                    
                    // Bottom section: Library - FIXED HEIGHT
                    if showLibrary {
                        Divider()
                            .background(Color.black.opacity(0.5))
                        
                        HStack(spacing: 0) {
                            // Library - FIXED DIMENSIONS
                            ShaderLibraryView(showLibrary: $showLibrary)
                                .frame(width: showConsole ? 800 : 1200, height: 250)
                            
                            // Console - FIXED DIMENSIONS
                            if showConsole {
                                Divider()
                                    .background(Color.black.opacity(0.5))
...
struct MetalPreviewView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            viewportSize = CGSize(width: 480, height: 400)  // Fixed size for stability
        }
    }
}
